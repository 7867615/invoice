...
export default async function DashboardPage() {
  ...
  const stats = {
    ...
    processingDocuments: documents?.filter((d) => d.status === "processing").length || 0,
    completedDocuments: documents?.filter((d) => d.status === "completed").length || 0,
    tokensUsed: profile ? 100 - profile.tokens_remaining : 0,
  }

  const maxDocuments = profile?.plan_type === "free" ? 15 : profile?.plan_type === "pro" ? 100 : 500
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Welcome back{profile?.full_name ? `, ${profile.full_name}` : ""}! Here's an overview of your document
          processing activity.
        </p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">{maxDocuments - stats.totalDocuments} remaining</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">{profile?.tokens_remaining || 0} remaining</p>
          </CardContent>
        </Card>
      </div>
